...
public class TarefaRepository extends AbstractRepository<Tarefa, Long> {
    ...
    public List<Tarefa> findByUsuario(Usuario usuario) {
        ...
        try {
            TypedQuery<Tarefa> query = em.createQuery(
                    "SELECT t FROM Tarefa t WHERE t.usuario = :usuario ORDER BY t.dataCriacao DESC", 
                    Tarefa.class);
    ...
    public List<Tarefa> findByUsuarioAndStatus(Usuario usuario, StatusTarefa status) {
        ...
        try {
            TypedQuery<Tarefa> query = em.createQuery(
                    "SELECT t FROM Tarefa t WHERE t.usuario = :usuario AND t.status = :status " +
    ...
    public List<Tarefa> findByUsuarioAndPrioridade(Usuario usuario, com.gerenciadortarefas.model.PrioridadeTarefa prioridade) {
        EntityManager em = JPAUtil.getEntityManager();
        try {
            TypedQuery<Tarefa> query = em.createQuery(
                    "SELECT t FROM Tarefa t WHERE t.usuario = :usuario AND t.prioridade = :prioridade " +
                    "ORDER BY t.dataCriacao DESC", 
                    Tarefa.class);
            query.setParameter("usuario", usuario);
